...
public class HeaderInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        boolean getPatternsByIds = request.getRequestURI().contains("/api/v1/pattern/by-ids") && ("GET".equalsIgnoreCase(request.getMethod()));
        boolean getPatternChapter = request.getRequestURI().contains("/api/v1/pattern/chapter/") && ("GET".equalsIgnoreCase(request.getMethod()));
        try {
            if (request.getRequestURI().contains("/swagger")
                    || (request.getRequestURI().contains("/pattern/availability"))
